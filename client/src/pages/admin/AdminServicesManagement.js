...
export default function AdminServicesManagement() {
  ...
  const handleUploadImage = async (service, file) => {
    ...
    const API_BASE = process.env.REACT_APP_API_URL || '';
    const fd = new FormData();
    fd.append('name', service.name || ''); fd.append('description', service.description || '');
    fd.append('category', service.category || ''); fd.append('duration', service.duration || '');
    fd.append('price', service.price || 0); fd.append('active', service.active ? 'true' : 'false');
  ...
  const handleCreateOrUpdate = async (e) => {
    e.preventDefault();
    const fd = new FormData();
    fd.append('name', formData.name); fd.append('description', formData.description);
    fd.append('category', formData.category); fd.append('duration', formData.duration);
    fd.append('price', formData.price); fd.append('active', formData.active ? 'true' : 'false');
    ...
    try {
      ...
      setFormData({ name: '', description: '', category: '', duration: '', price: '', image: null, active: true });
    } catch (err) {
        ...
        <main className="p-8 animate-[fadeIn_0.3s_ease-in]">
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-[#6B6B6B] text-[14px]">{services.length} services · {categories.length - 1} categories</p>
            </div>
            <button
