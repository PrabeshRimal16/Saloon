...
const CSS = `
  ...
  .btn-pill-gold:hover { transform: translateY(-2px); box-shadow: 0 14px 40px rgba(184,150,12,0.5); }
  @keyframes dash-shimmer { to { background-position: 200% center; } }
  ...
  .svc-card { background: white; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.08); transition: box-shadow 0.3s, transform 0.3s; }
  .svc-card:hover { box-shadow: 0 16px 40px rgba(0,0,0,0.14); transform: translateY(-8px); }
  .svc-img-wrap { aspect-ratio: 4/5; min-height: 300px; position: relative; overflow: hidden; background: linear-gradient(135deg, #1C1C1E 0%, #2C2416 100%); display: flex; align-items: center; justify-content: center; }
  ...
  .btn-svc-book { padding: 12px 24px; border-radius: 8px; background: linear-gradient(90deg, #B8960C 0%, #D4AF37 100%); color: white; border: none; cursor: pointer; font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; transition: opacity 0.2s, transform 0.2s; min-height: 44px; }
...
function SkeletonServiceCard() {
  return (
    <div className="skeleton-card" style={{ borderRadius: 16 }}>
      <div className="skeleton skeleton-img" style={{ minHeight: 300, aspectRatio: '4/5' }} />
      <div className="skeleton-body">
        <div className="skeleton skeleton-line lg" />
        <div className="skeleton skeleton-line" />
        <div className="skeleton skeleton-line sm" />
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 16 }}>
          <div className="skeleton skeleton-line" style={{ width: 70, height: 20, marginBottom: 0 }} />
          <div className="skeleton skeleton-line" style={{ width: 100, height: 36, marginBottom: 0, borderRadius: 8 }} />
        </div>
      </div>
    </div>
  );
...
export default function CustomerDashboard() {
  ...
  const [servicesLoading, setSvcsLoad]  = useState(true);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (mounted) setSvcsLoad(false);
      }
    })();
    fetch(`${API_BASE}/api/offers`)
      .then(r => r.json())
      .then(d => mounted && setOffers((Array.isArray(d) ? d : []).filter(o => !o.valid_until || new Date(o.valid_until) > new Date()).slice(0, 5)))
      .catch(() => {});
    return () => { mounted = false; };
  }, []);

  const openBookingForService = useCallback((svc) => {
    setPreSelectedIds([svc.id]);
    setModalOpen(true);
  }, []);
      ...
      <main style={{ flexGrow: 1, paddingTop: 72 }}>
        ...
        <section className="hero-section">
          ...
          <div className="hero-content">
            ...
            <h1 className="hero-h1">Look &amp;<br />Feel Stunning.</h1>
            <p className="hero-sub">Experience luxury grooming tailored to your unique style at The Salon At Reston.</p>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16 }}>
              <button className="btn-pill-gold" onClick={() => { setPreSelectedIds([]); setModalOpen(true); }}>
        ...
        <div style={{ background: '#FFFFFF' }}>
          <div className="section-wrap">
            <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 48 }}>
              ...
              <button onClick={() => navigate('/services')} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'DM Sans,sans-serif', fontSize: 13, fontWeight: 600, color: '#B8960C', textTransform: 'uppercase', letterSpacing: '1px', transition: 'gap 0.2s', minHeight: 44 }}>
            ...
            <div className="services-grid">
              {servicesLoading
                ? [1, 2, 3].map(n => <SkeletonServiceCard key={n} />)
                      ...
                      <div key={s.id || i} className="svc-card">
                        ...
                        <div className="svc-body">
                          ...
                          <div className="svc-footer">
                            ...
                            <button
                              className="btn-svc-book"
                              onClick={() => s.id ? openBookingForService(s) : navigate('/appointments')}
                            >
                              Book Now
                            </button>
                          </div>
          ...
          <div style={{ background: '#F8F7F5' }}>
            <div className="section-wrap">
              <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 40 }}>
                ...
                <button onClick={() => navigate('/offers')} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'DM Sans,sans-serif', fontSize: 13, fontWeight: 600, color: '#B8960C', textTransform: 'uppercase', letterSpacing: '1px', minHeight: 44 }}>
