            ...
            {loading ? (
              ...
              <>
                {(user && (user.avatar_url || user.photo || user.avatar || user.avatarUrl || user.picture)) ? (
                  <img
                    src={user.avatar_url || user.photo || user.avatar || user.avatarUrl || user.picture}
                    alt={user.name || user.email}
                    className={`${collapsed ? 'w-[36px] h-[36px]' : 'w-[44px] h-[44px]'} rounded-full object-cover border-2 border-[#C9A84C] shrink-0`}
